...
<body>
        ...
        <div class="topbar-inner">
            <div style="display:flex;align-items:center;gap:12px">
                ...
                <div style="display:flex;flex-direction:column">
                    <div style="font-weight:700">Edit Equipment</div>
                    <div style="font-size:12px;color:var(--muted)">Inventory / Edit</div>
                </div>
            </div>
                ...
                <form method="POST" action="/inventory/{{ $item->id }}/update" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        <div class="field">
                            ...
                            <input id="name" name="name" type="text" value="{{ $item->name }}" required placeholder="e.g., Portable Radio">
                            <small class="helper">Use a clear, descriptive name.</small>
                        </div>

                        <div class="field">
                            <label for="quantity">Quantity</label>
                            <div style="display:flex;align-items:center;gap:8px">
                                <button type="button" id="qty-decr" class="btn">-</button>
                                <input id="quantity" name="quantity" type="number" min="0" value="{{ $item->quantity }}" style="width:78px;text-align:center">
                                <button type="button" id="qty-incr" class="btn">+</button>
                            </div>
                            <small class="helper">Available stock count.</small>
                        </div>

                        <div class="field">
                            <label for="category">Category</label>
                            <input id="category" name="category" type="text" value="{{ $item->category }}" placeholder="e.g., Communications">
                        </div>

                        <div class="field">
                            <label for="type">Type</label>
                            <select id="type" name="type">
                                <option value="Non-consumable" {{ $item->type=='Non-consumable' ? 'selected' : '' }}>Non-consumable</option>
                        ...
                        <div class="field">
                            ...
                            <div class="file-input">
                                ...
                                <div class="image-preview" id="image-preview">
                                    @if($item->image_path)
                                        <img src="{{ asset('storage/'.$item->image_path) }}" alt="Preview" style="width:120px;height:80px;object-fit:cover;border-radius:8px;border:1px solid #e6e9ef">
                                    @else
                                        <div style="width:120px;height:80px;border-radius:8px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;color:#94a3b8">No image</div>
                                    @endif
                                </div>
                            </div>
                            <small class="helper">Upload an image (optional).</small>
                        </div>

                        <div class="field full">
                            <label for="notes">Notes</label>
                            <textarea id="notes" name="notes">{{ $item->notes }}</textarea>
                            <small class="helper">Short notes or condition details.</small>
                        </div>
                    </div>

                    <div class="actions"><a href="/inventory" class="btn">Cancel</a><button type="submit" class="btn primary">Save Changes</button></div>
                </form>
            </div>
        </main>
    </div>
    ...
    <script>
        (function(){
            // Small form interactions: quantity +/- and image preview
