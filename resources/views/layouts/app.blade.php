...
<head>
    ...
    <link rel="shortcut icon" href="{{ asset('images/favi.png') }}" type="image/png">
    <link rel="apple-touch-icon" href="{{ asset('images/favi.png') }}">
...
<body>
    ...
    <div class="app">
        @php
            // Ensure `pendingAccounts` exists for pages that don't set it
        ...
        <aside class="sidebar {{ (request()->is('vehicle/maintenance*') || request()->is('vehicle/monitoring*')) ? 'closed' : '' }}" id="sidebar">
            ...
            <nav class="nav">
                ...
                <a href="/accounts?tab=accounts" class="{{ request()->is('accounts*') ? 'active' : '' }}"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 12a5 5 0 100-10 5 5 0 000 10zM4 20a8 8 0 0116 0v1H4v-1z" fill="currentColor"/></svg><span class="label">Accounts</span>@if(!empty($pendingAccounts) && (int)$pendingAccounts > 0)<span class="sidebar-badge" style="margin-left:8px">{{ (int)$pendingAccounts }}</span>@endif</a>
